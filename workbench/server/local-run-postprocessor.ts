...
export class LocalRunPostprocessor implements AgentRunPostprocessor {
  ...
  process(input: AgentRunPostprocessorInput) {
    ...
    const stored = this.input.evidenceStore.write({
      ...
      run: { id: input.runId, adapterId: input.adapterId, startSha: input.startSha, revisionOfProposalId: input.revisionOfProposalId, isolation: input.attestation.isolation, networkEgress: input.attestation.networkEgress, productionEligible: input.attestation.productionEligible, runtimeAttestationDigest: input.attestation.attestationDigest, stdoutDigest: input.stdoutDigest, stderrDigest: input.stderrDigest,
        // How many model calls and tokens produced this change, and which tools it took; counts and names only.
        // Both come from the builder's own protocol, so they say what it reported, not what was independently metered.
        modelUsage: input.modelUsage ? { ...input.modelUsage, reportSource: 'agent_protocol', independentlyObserved: false } : null,
        toolSteps: input.toolSteps ?? null },
      checks,
      criteriaCoverage,
      testProvenance,
