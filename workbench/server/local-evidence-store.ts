...
export type StoredEvidencePackage = {
  ...
  run: { id: string; adapterId: string; startSha?: string; revisionOfProposalId?: string; isolation: 'unisolated_process' | 'container'; networkEgress: 'denied' | 'allowlist' | 'unrestricted'; productionEligible: boolean; runtimeAttestationDigest?: string; stdoutDigest?: string; stderrDigest?: string
    /** Null when the builder did not report usage; token figures are only as complete as `callsWithUsage` says. */
    modelUsage?: { modelCalls: number; httpRequests: number; callsWithUsage: number; promptTokens: number; completionTokens: number; totalTokens: number; peakContextTokens: number; contextBudgetTokens?: number; reportSource: 'agent_protocol'; independentlyObserved: false } | null
    toolSteps?: { total: number; byTool: Record<string, number>; unsuccessful: number; reportSource: 'agent_protocol'; independentlyObserved: false } | null }
