...
import type { LocalActor, LocalActorUpdate, LocalAgentModelUsage, LocalAgentRunDetail, LocalAgentRuntimeDescriptor, LocalAgentToolStep, LocalWorkItem, LocalCodeHostConnection, LocalProject, LocalProjectInput, LocalProjectRole, LocalIdentityMode, LocalChangeProposal, LocalEvidencePackageView, LocalIntentVersion, LocalReviewAssignment, LocalReviewReadiness } from './local-control-plane-client'
import './styles.css'
...
function ContextPage() {
  ...
  return (
    <>
      ...
      />
      <DemoRegion title="上下文面板" note="本页的上下文来源、检索质量与预算面板仍为演示数据；真实的“声明 vs 实际读取”对账在「Agent Runs」页每个 Run 的「运行详情」里。">
...
function LocalRunContextDrawer({ detail, onClose }: { detail: LocalAgentRunDetail; onClose: () => void }) {
  ...
  // Steps and usage come from the event log once the builder has exited, and from its progress file while it runs.
  const toolSteps: LocalAgentToolStep[] = detail.liveToolSteps ?? detail.events.filter((event) => event.eventType === 'agent_run.tool_step').map((event) => event.payload as unknown as LocalAgentToolStep)
  const usage = detail.liveModelUsage ?? detail.events.filter((event) => event.eventType === 'agent_run.model_usage').at(-1)?.payload as unknown as LocalAgentModelUsage | undefined
  const budgetExceeded = detail.events.find((event) => event.eventType === 'agent_run.context_budget_exceeded')?.payload
  const stepOutcomeLabels: Record<LocalAgentToolStep['outcome'], string> = { ok: '成功', failed: '非零退出', error: '被拒绝 / 出错', not_run: '未执行' }
  return (
    <>
      <button className="local-evidence-overlay" aria-label="关闭运行详情" onClick={onClose} />
      <aside className="local-evidence-drawer">
        <header><div><span className="eyebrow">运行详情 · 上下文对账</span><h3>{detail.agentRun.id}</h3><p>Manifest 是声明，实际读取是事实，两者不一致本身就是审查信息；每一步工具调用与模型用量列在下方。</p></div><button className="icon-button" onClick={onClose} aria-label="关闭"><X size={15} /></button></header>
        ...
        <section><span>工具调用步骤{detail.liveToolSteps ? ' · 运行中，每 3 秒刷新' : ''}</span>{toolSteps.length ? <div className="local-context-rows" role={detail.liveToolSteps ? 'status' : undefined}>{toolSteps.map((step, index) => <div className={`local-context-row ${step.outcome === 'ok' ? 'declared' : step.outcome === 'not_run' ? 'unread' : 'rejected'}`} key={`${step.step}-${index}`}>
          <strong>#{step.step} {step.tool}{step.path ? ` · ${step.path}` : ''}</strong>
          <span className={`local-check-provenance ${step.outcome === 'ok' ? 'pre_existing' : step.outcome === 'not_run' ? 'all_tests' : 'unverified'}`}>{stepOutcomeLabels[step.outcome] ?? step.outcome}{step.exitCode !== undefined ? ` · exit ${step.exitCode ?? '—'}` : ''}</span>
          {step.command && <code>{step.command}</code>}
          {step.error && <em>{step.error}</em>}
        </div>)}</div> : <p className="local-evidence-empty">{detail.agentRun.status === 'running' ? '尚未上报任何工具调用。' : '这个 Builder 没有上报工具调用步骤（Codex / Claude Code 包装器目前不上报）。'}</p>}<small>只记录工具、路径或命令与结果，不记录文件内容和命令输出；来自 Agent 自述协议，未被独立观测。</small></section>
        <section><span>模型用量</span>{usage ? <div className="local-evidence-runtime"><code>{usage.modelCalls} 次模型调用 · {usage.totalTokens} tokens（输入 {usage.promptTokens} · 输出 {usage.completionTokens}）</code><small>{usage.callsWithUsage}/{usage.modelCalls} 次调用带 Provider 用量报告 · {usage.httpRequests} 次 HTTP 请求（含重试） · 峰值上下文约 {usage.peakContextTokens}{usage.contextBudgetTokens ? ` / 预算 ${usage.contextBudgetTokens}` : ''} tokens</small></div> : <p className="local-evidence-empty">{detail.agentRun.status === 'running' ? '用量在 Builder 结束时上报。' : '这个 Builder 没有上报模型用量。'}</p>}{budgetExceeded && <p className="local-form-error" role="alert">上下文超出预算：第 {String(budgetExceeded.step)} 步约 {String(budgetExceeded.estimatedTokens)} tokens，超过 {String(budgetExceeded.budgetTokens)} tokens 预算，请求未发送给 Provider。</p>}</section>
...
function LocalAgentRuns() {
  ...
  const [error, setError] = useState<string>()
  // An open drawer on a queued or running run follows it, so each tool call appears as the builder reports it.
  const openRunId = detail?.agentRun.id
  const openRunInFlight = detail?.agentRun.status === 'queued' || detail?.agentRun.status === 'running'
  const { getAgentRunDetail } = local
  useEffect(() => {
    if (!openRunId || !openRunInFlight) return
    const timer = window.setInterval(() => {
      getAgentRunDetail(openRunId).then((next) => setDetail((current) => current?.agentRun.id === openRunId ? next : current)).catch(() => undefined)
    }, 3000)
    return () => window.clearInterval(timer)
  }, [openRunId, openRunInFlight, getAgentRunDetail])
  if (local.status !== 'ready') return null
...
function LocalReviewQueue() {
  ...
  const act = async (proposal: LocalChangeProposal, action: 'refresh' | 'approved' | 'changes_requested' | 'merge' | 'revise' | 'reject' | { overrideCriterionId: string }) => {
    ...
    try {
      ...
      else await local.reviewChangeProposal(proposal, action, notes[proposal.id]?.trim() || (action === 'approved' ? (humanCriteriaFor(proposal.id).length || proposal.policyFiles?.length ? '' : '本地证据与 Revision 已审查。') : '请根据审查意见修改后提交新 Revision。'))
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught))
    } finally {
      setBusyId(undefined)
    ...
    <section className="panel local-review-queue">
      ...
      {local.changeProposals.length === 0 ? <div className="local-empty"><GitBranch size={18} /><p>暂无本地 Change Proposal。请从具有独立 Base / Head Revision 的真实 Git 仓库创建。</p></div> : (
        <div className="local-proposal-list">{local.changeProposals.map((proposal) => {
          ...
          const unsignedHumanCriteria = humanCriteria.length > 0 && !notes[proposal.id]?.trim()
