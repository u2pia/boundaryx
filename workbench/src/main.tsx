...
function LocalReviewQueue() {
  ...
  const act = async (proposal: LocalChangeProposal, action: 'refresh' | 'approved' | 'changes_requested' | 'merge' | 'host_merge' | 'revise' | 'reject' | { overrideCriterionId: string }) => {
    ...
    try {
      ...
      else if (action === 'host_merge') await local.mergeChangeProposal(proposal.id, { onHost: true })
      ...
      {evidencePreview && <>
        ...
        <aside className="local-evidence-drawer">
          ...
          <section><span>Checks</span>{evidencePreview.evidencePackage.checks.length ? evidencePreview.evidencePackage.checks.map((check) => <article className={`local-evidence-check ${check.conclusion}`} key={check.id}>
            <div><strong>{check.name}</strong><em>{check.kind ?? 'test'} · {check.conclusion} · {check.durationMs}ms · exit {check.exitCode ?? 'n/a'}</em>{check.provenance && <span className={`local-check-provenance ${check.provenance}`} title={check.provenance === 'pre_existing' ? '测试目录已重置回 Base，本次变更无法影响这条结论' : check.provenance === 'all_tests' ? '包含本次变更自带的测试，不能单独作为关键验收标准的证据' : check.provenance === 'isolated' ? '评分器取自 Base，被测代码在沙箱中运行且读不到 Holdout，Builder 也读不到：独立证据' : check.provenance === 'isolated_partial' ? 'Holdout 评测缺少被测代码沙箱或 Builder 隔离，被测代码或 Builder 可能读到 Holdout' : 'Manifest 未声明 testPaths，无法判定来源'}>{check.provenance === 'pre_existing' ? '既有测试' : check.provenance === 'all_tests' ? '含本次新增' : check.provenance === 'isolated' ? '隔离评测' : check.provenance === 'isolated_partial' ? '隔离不完整' : '来源未验证'}</span>}</div>
