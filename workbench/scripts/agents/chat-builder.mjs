...
const maxSteps = Number(process.env.APERTURE_BUILDER_MAX_STEPS) || 60
...
Start by listing and reading the files you need. Make the change with write_file or replace_in_file, add or
...
async function complete(messages) {
  const body = { model, messages, tools, tool_choice: 'auto', ...(process.env.APERTURE_AGENT_REASONING_EFFORT ? { reasoning_effort: process.env.APERTURE_AGENT_REASONING_EFFORT } : {}) }
...
try {
  for (let step = 1; step <= maxSteps && summary === undefined; step += 1) {
    ...
    for (const call of calls) {
      ...
      console.error(`[chat-builder] step ${step} ${name}${args.path ? ` ${args.path}` : args.command ? ` ${String(args.command).slice(0, 120)}` : ''}`)
