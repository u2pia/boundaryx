...
| E-H4 | 中→低 | Builder 留下守护进程，在 Control Plane 提交 Head 之后替换评分器，退出前再换回 | **残余，接受**：E-H1 之后该路径最多让作弊看起来是 `self_graded`，仍需 Owner 审阅推翻。彻底解决需要在全新 checkout 里跑 Check，或杀掉 Builder 的进程组 |
| E-M1 | 中→低 | 泄漏检测可被绕过：内容以 `++ ` 开头的新增行被当成文件头；`.gitignore` 或 `.git/info/exclude` 里的文件不进 Diff；还有拼接或编码后的答案 | **残余，接受**：同上，只影响 `failed` 与 `self_graded` 的区别，不影响能否走到 `passed`。README 与契约已写明「只认原样拷贝」 |
...
| C-H2 | 高 | 事件 Digest 是无密钥 SHA-256：持有数据库文件的人可以按规则追加一串自洽的伪造事件，同时插入对应的状态行，上述绑定会全部通过 | **未修复，计划中**：密钥封印（HMAC，密钥不放在数据库里，启动时回填），并把链头外部锚定。README 与契约已写明当前只防「绕过平台直接改行」，不防「掌握算法的数据库持有者」 |
...
1. **C-H2 密钥封印**：新增 `event_seals` 表，对每个事件 Digest 做 HMAC，密钥来自环境变量或数据库之外的文件。启动时为历史事件回填封印，合并与 Merge Evidence 读取时校验。需要同时设计密钥轮换，以及密钥缺失时的降级提示。
2. **隔离评估器**：被测代码运行在无法访问 Dataset 的沙箱（容器 Runtime 不挂载 Dataset），评分由外部上报，这是恢复 Evaluation 独立性的唯一途径。同时解决 E-H4 与 E-M1。
